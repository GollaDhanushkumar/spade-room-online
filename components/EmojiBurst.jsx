...
// Returns { activeReactions, sendReaction }
export function useEmojiReactions({ roomCode, myPlayerId }) {
  ...
  const sendReaction = useCallback(
    (targetId, emoji, fromName) => {
      ...
      // Also show locally because broadcast may not echo back to sender
      const ts = Date.now();
...
export function EmojiPicker({ targetPlayerId, targetName, onPick, onClose, anchorRect }) {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (top < 8) {
      top = anchorRect ? anchorRect.bottom + 12 : 100; // below if not enough room above
    }
  }
