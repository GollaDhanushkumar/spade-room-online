...
// 8 avataaars + 8 personas + 8 dylan = 24 total
const AVATAAARS_SEEDS = ['ace', 'blaze', 'jet', 'nova', 'orbit', 'rex', 'sage', 'zen'];
const PERSONAS_SEEDS = ['fox', 'tiger', 'panda', 'rabbit', 'lion', 'otter', 'koala', 'bear'];
const DYLAN_SEEDS = ['sky', 'mint', 'rose', 'amber', 'coral', 'jade', 'plum', 'wave'];
...
export const DICEBEAR_AVATARS = [
  ...
  ...PERSONAS_SEEDS.map((seed) => ({
    id: `dicebear:personas:${seed}`,
    name: seed,
    style: 'personas',
    url: dicebearUrl('personas', seed),
  })),
  ...DYLAN_SEEDS.map((seed) => ({
    id: `dicebear:dylan:${seed}`,
    name: seed,
    style: 'dylan',
    url: dicebearUrl('dylan', seed),
  })),
];
