...
export const FRIEND_AVATARS = [
  ...
  { id: 'friend:sindhu', name: 'Sindhu', file: '/avatars/friends/sindhu.jpeg' },
...
// All avatars in one list (friends first, then DiceBear)
export const ALL_AVATARS = [...FRIEND_AVATARS, ...DICEBEAR_AVATARS];
...
export function resolveAvatar(id) {
  ...
  if (av.id.startsWith('friend:')) {
    return { type: 'image', src: av.file, name: av.name, isFriend: true };
  }
