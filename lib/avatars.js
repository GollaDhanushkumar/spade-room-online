...
export const FRIEND_AVATARS = [
  ...
  { id: 'friend:bhavana', name: 'Bhavana', file: '/avatars/friends/bhavana.jpeg', secretFlipFile: '/avatars/friends/bhavana-secret.png' },
  { id: 'friend:sindhu', name: 'Sindhu', file: '/avatars/friends/sindhu.jpeg', secretFlipFile: '/avatars/friends/sindhu-secret-v2.png' },
...
// All avatars in one list (friends first, then secrets, then animals, then DiceBear)
// Secrets included so getAvatarById works for other players seeing them
export const ALL_AVATARS = [...FRIEND_AVATARS, ...SECRET_AVATARS, ...ANIMAL_AVATARS, ...DICEBEAR_AVATARS];
...
export function pickRandomAvatar(excludeIds = []) {
  ...
  const available = [...ANIMAL_AVATARS, ...DICEBEAR_AVATARS].filter((a) => !excluded.has(a.id));
