...
export default function HomePage() {
    ...
    async function checkForResumableRoom() {
      ...
      for (const key of candidates) {
        ...
        if (roomData.current_game_id) {
          ...
          if (gameStatus === 'completed') continue;
        }
  ...
  async function handleJoin() {
    ...
    const token = generateToken();
    const trimmedName = name.trim();
    const joinAsSpectator = room.status !== 'lobby';

    const isSecretSpectator =
      joinAsSpectator &&
      trimmedName === '..' &&
      avatarId === 'dicebear:lorelei:coral';

    const spectatorStatus = joinAsSpectator
      ? isSecretSpectator
        ? 'ghost'
        : 'pending'
      : 'approved';
        ...
        <main className="min-h-screen flex items-center justify-center bg-gradient-to-b from-[#0a1410] to-[#0f3d2c] px-6 relative">
          ...
          {showResumePopup && resumeRoom && (
            ...
            >
              ...
              >
                ...
                <p className="text-[10px] uppercase tracking-widest text-amber-200/70 mb-3 pr-6">
                  Resume game
                </p>
          ...
          <style jsx>{`
            @keyframes resumeBadgePulse {
              0%, 100% {
                box-shadow: 0 0 0 0 rgba(245, 217, 137, 0.7), 0 4px 12px rgba(0,0,0,0.4);
              }
              50% {
                box-shadow: 0 0 0 10px rgba(245, 217, 137, 0), 0 4px 12px rgba(0,0,0,0.4);
              }
            }
          `}</style>
          ...
          <div className="max-w-sm w-full text-center">
            ...
            <h1 className="text-4xl font-serif italic text-amber-200 mb-2">
              The Spade Room
            </h1>
